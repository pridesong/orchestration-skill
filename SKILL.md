...
```json
{
  ...
  "rules": [
    "Stage 0 初始化：建任务目录 tasks/<task_id>/{artifacts,feedback}",
    "Stage 0.5 编排生成：把 data 填入 templates/orchestrator.t3.json 的 data 槽，整个模板作为 task 字符串派给编排者 subagent（唯一派发方式，禁止散文包装），编排者产出三件套写入任务目录",
...
python scripts/validate.py         <task_dir>           # 三件套机械校验
